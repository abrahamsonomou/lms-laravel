...
<div class="row">
    <div class="col-md-6 mb-3">
        ...
        <input type="text" id="code" name="code" value="{{ old('code', $organisation->code ?? '') }}"
               class="form-control @error('code') is-invalid @enderror" required>
        @error('code')<div class="invalid-feedback">{{ $message }}</div>@enderror
    </div>
    <div class="col-md-6 mb-3">
        <label for="nom" class="form-label">Nom <span class="text-danger">*</span></label>
        <input type="text" id="nom" name="nom" value="{{ old('nom', $organisation->nom ?? '') }}"
               class="form-control @error('nom') is-invalid @enderror" required>
        @error('nom')<div class="invalid-feedback">{{ $message }}</div>@enderror
    </div>
    <div class="col-md-6 mb-3">
        <label for="email" class="form-label">Email</label>
        <input type="email" id="email" name="email" value="{{ old('email', $organisation->email ?? '') }}"
               class="form-control @error('email') is-invalid @enderror">
        @error('email')<div class="invalid-feedback">{{ $message }}</div>@enderror
    </div>
    <div class="col-md-6 mb-3">
        <label for="telephone" class="form-label">Téléphone</label>
        <input type="text" id="telephone" name="telephone" value="{{ old('telephone', $organisation->telephone ?? '') }}"
    ...
    <div class="col-12 mb-3">
        ...
        <textarea id="adresse" name="adresse" rows="2"
                  class="form-control @error('adresse') is-invalid @enderror">{{ old('adresse', $organisation->adresse ?? '') }}</textarea>
    ...
    <div class="col-md-6 mb-3">
        ...
        <select id="pays_id" name="pays_id" class="form-select @error('pays_id') is-invalid @enderror">
            ...
            @foreach ($pays as $p)
                <option value="{{ $p->id }}" @selected(old('pays_id', $organisation->pays_id ?? '') == $p->id)>{{ $p->nom }}</option>
            @endforeach
        </select>
    ...
    <div class="col-md-6 mb-3">
        ...
        <select id="ville_id" name="ville_id" class="form-select @error('ville_id') is-invalid @enderror">
            <option value="">— Aucun(e) —</option>
            @foreach ($villes as $v)
                <option value="{{ $v->id }}" @selected(old('ville_id', $organisation->ville_id ?? '') == $v->id)>{{ $v->nom }}</option>
            @endforeach
        </select>
    ...
    <div class="col-md-6 mb-3">
        ...
        <select id="devise_id" name="devise_id" class="form-select @error('devise_id') is-invalid @enderror">
            ...
            @foreach ($devises as $d)
                <option value="{{ $d->id }}" @selected(old('devise_id', $organisation->devise_id ?? '') == $d->id)>{{ $d->code }} — {{ $d->nom }}</option>
            @endforeach
        </select>
    ...
    <div class="col-md-6 mb-3">
        ...
        <select id="langue_id" name="langue_id" class="form-select @error('langue_id') is-invalid @enderror">
            <option value="">— Aucun(e) —</option>
            @foreach ($langues as $l)
                <option value="{{ $l->id }}" @selected(old('langue_id', $organisation->langue_id ?? '') == $l->id)>{{ $l->nom }}</option>
            @endforeach
        </select>
        ...
        <div class="d-flex align-items-center gap-3">
            @if (isset($organisation) && $organisation->logo)
                <img src="{{ \Illuminate\Support\Facades\Storage::disk('public')->url($organisation->logo) }}"
                     alt="{{ $organisation->nom }}" class="rounded" width="64" height="64" style="object-fit: cover;">
            @endif
    ...
    <div class="col-md-6 mb-3">
        ...
        <div class="form-check form-switch mt-2">
            ...
            <input type="checkbox" id="active" name="active" value="1" class="form-check-input"
                   @checked(old('active', $organisation->active ?? true))>
            <label class="form-check-label" for="active">Organisation active</label>
        </div>
    </div>
</div>
...
<div class="d-flex gap-2">
    ...
    <a href="{{ route('admin.organisations.index') }}" class="btn btn-outline-secondary">Annuler</a>
</div>
